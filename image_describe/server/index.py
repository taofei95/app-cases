...
def call_handler_upload():
    event = Event()
    context = Context()

    response_data = handler.handle_upload(event, context)    
    res = format_response(response_data)
    return res

@app.route('/web_image', methods=['POST'])
def call_handler_web():
    event = Event()
    context = Context()

    response_data = handler.handle_web(event, context)
    res = format_response(response_data)
    return res
